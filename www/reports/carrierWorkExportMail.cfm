...
<cfset Subject = request.qGetSystemSetupOptions.WorkExpHead>
	...
	<cfif form.MailTo is not "" AND form.MailFrom is not "" AND form.Subject is not "">
		...
		<cfoutput>
			...
			<cftry>
				...
				#form.body#
						 <cfmailparam
							file="#careerReport.loadnumber#Export.pdf"
				...
				<cfinvoke component="#variables.objloadGateway#" method="setLogMails" loadID="#loadID#" date="#Now()#" subject="#form.Subject#" emailBody="#form.body#" reportType="exportWork" fromAddress="#SmtpUsername#" toAddress="#form.MailTo#" />
				...
				<script>
					setTimeout(function(){ 
						...
						if(emailDisplayStatus == '1'){
							window.opener.document.getElementById('dispatchHiddenValue').value="Emailed Export Work Order >";
...
<p> 
<form action = "index.cfm?event=loadMail&type=exportWork&loadid=#loadID#&#session.URLToken#" method="POST">
	<div class="white-mid" style=" border-top: 5px solid rgb(130, 187, 239);width: 100%;">
		<div class="form-con" style="width:auto;">
			<fieldset>
			<div style="color:##000000;font-size:14px;font-weight:bold;margin-bottom:20px;margin-top:10px;">Carrier Work Order Export Mail</div>
			<div class="clear"></div>
