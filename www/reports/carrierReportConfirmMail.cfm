	...
	<cfif form.MailTo is not "" AND form.MailFrom is not "" AND form.Subject is not "">
		...
		<cfoutput>
			<cfif variables.frieghtBroker eq 'Carrier'>
				<cfreport name="genPDF" format="PDF" template="loadReportForCarrierConfirmation.cfr" style="../webroot/styles/reportStyle.css" query="#careerReport#"> 
					<cfreportParam name="ReportDate" value="#DateFormat(Now(),'mm/dd/yyyy')#"> 
				</cfreport>
				<cfset variables.pdfName='CarrierReport'>
			<cfelseif variables.frieghtBroker eq 'Dispatch'>
				<cfreport name="genPDF" format="PDF" template="LoadReportForDispatch.cfr" style="../webroot/styles/reportStyle.css" query="#careerReport#"> 
					<cfreportParam name="ReportDate" value="#DateFormat(Now(),'mm/dd/yyyy')#"> 
				</cfreport>
				<cfset variables.pdfName='DispatchReport'>
			</cfif>	
			<cftry>
				...
				#form.body#
						 <cfmailparam
							file="#careerReport.LOADNUMBER#.#variables.pdfName#.pdf"
...
<p> 
<form action = "index.cfm?event=loadMail&type=#variables.frieghtBroker#&loadid=#loadID#&#session.URLToken#" method="POST">
	<div class="white-mid" style=" border-top: 5px solid rgb(130, 187, 239);width: 100%;">
		<div class="form-con" style="width:auto;">
			<fieldset>
			<div style="color:##000000;font-size:14px;font-weight:bold;margin-bottom:20px;margin-top:10px;">#variables.frieghtBroker# Report</div>
			<div class="clear"></div>
