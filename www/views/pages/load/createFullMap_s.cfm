		...
		geocoder.geocode({latLng: latlng},function(res,stat){
			if(stat==google.maps.GeocoderStatus.OK){
				...
				marker = new google.maps.Marker({
					...
					map: map,
					position: mark,
					icon: '../../../webroot/images/red-truck.png',
				});
				<cfoutput>
				p = marker_num;
				google.maps.event.addListener(marker, 'click', (function(marker, p) {
					return function() {
				    	infowindow.setContent("<div style='height:100px;width:500px;'> Load No. : #URL.loadNum#<br/> Equipment Name: Test EQP<br/> Driver Name: #url.currLocDiver# <br/> Date/Time: #url.currLocDateTime# <br/> Address:" + res[0].formatted_address + " </div>");
				    	infowindow.open(map, marker);
					}
				})(marker, p));
				</cfoutput>
